...
def query(queries):
    ...
    for token in queries:
        if token not in stops:
            if token in corpes:
                ...
                #update doc_dict to total score for every term
                postings_lists = getScores(corpes[token])
                #calculating score for all documents
                #q_vector = [t1_idf_score * idf, t2_idf_score * idf, ...] len(query)
    ...
    for id in docid_set:
        ...
        for token in queries:
            if token in corpes:
                if token not in stops:
                    postings_list = getScores(corpes[token])
